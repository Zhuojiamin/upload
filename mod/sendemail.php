...
$M_db= new Mailconfig(
	array(
		...
		'user'	=> $db_ml['smtpuser'],
		'pass'	=> $db_ml['smtppass'],
		'smtphelo'=>$db_ml['smtphelo'],
		'smtpmxmailname' =>$db_ml['smtpmxmailname'],
		'mxdns'=>$db_ml['mxdns'],
		'mxdnsbak'=>$db_ml['mxdnsbak']
	)
);
Class Mailconfig {
	var $S_method = 1;
...
function sendemail($toemail,$subject,$message,$additional){
	global $M_db,$db_blogname,$rg_name,$db_blogurl,$windid,$winduid,$timestamp,$regpwd,$manager,$db_ceoemail,$fromemail,$pwd_user,$submit,$receiver,$old_title,$fid,$tid,$pwuser,$db_charset,$sendtoname;
	...
	if($M_db->S_method == 1){
		...
		if(substr(fgets($fp,512),0,3) != 250){
			Showmsg('email_from_failed');
		}
		fwrite($fp, "RCPT TO: <$toemail>\r\n");
		if(substr(fgets($fp,512),0,3) != 250){
			Showmsg('email_toemail_failed');
		}
		fwrite($fp, "DATA\r\n");
		if(substr(fgets($fp,512),0,3) != 354){
			Showmsg('email_data_failed');
		}
		$subject = str_replace("\n",' ',$subject);
		$msg  = "Date: ".Date("r")."\r\n";
