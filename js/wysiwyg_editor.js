...
function setfee(type){
	GE("ordinary_fee").disabled = type;
	GE("express_fee").disabled = type;
}
...
function insertsale(){
	...
	var code  = '[payto]';
	code += '(seller)' + GE("seller").value + '(/seller)';
	code += '(subject)' + GE("subject").value + '(/subject)';
	code += '(body)' + GE("saledes").value + '(/body)';
	code += '(price)' + GE("price").value + '(/price)';
	code += '(ordinary_fee)' + GE("ordinary_fee").value + '(/ordinary_fee)';
	code += '(express_fee)' + GE("express_fee").value + '(/express_fee)';
	code += '(demo)' + GE("demo").value + '(/demo)';
	code += '(contact)' + GE("contact").value + '(/contact)';
...
function smilepath(NewCode){
	return '<img src="' + imgpath + '/smile/' + smile[NewCode] + '" smile="' + NewCode + '" /> ';
}
...
function addsmile(NewCode){
	...
	if(editor._editMode=='textmode'){
		...
		sm = '<img src="' + imgpath + '/smile/' + smile[NewCode] + '" smile="' + NewCode + '" />';
		editor.insertHTML(sm);
	}
}
...
function insertform(id){
	...
	var ds   = GE('formstyle').getElementsByTagName('tr');
...
function showform(formid){
	...
	send_request('ajaxuser.php',read.get,'action=extend&type=setform&formid='+formid);
}

function extend(cmdID){
	if(typeof read == 'object' && read.obj != null && read.obj.id == 'wy_'+cmdID){
		read.close();read.obj=null;
	} else{
		read.obj = GE('wy_'+cmdID);
		send_request('ajaxuser.php',read.get,'action=extend&type='+cmdID);
	}
}
