...
function AddCommentResponse(){
	...
	if (msg[0] == 'cmt_empty') {
		...
		if (msg[1]) {
			...
			var replycmt = ' <a href="javascript:;" style="cursor:pointer;" onclick="ShowCmtReplyBox(\'' + msg[3] +'\',\''+ msg[1] +'\',\'' + msg[9] +'\',\''+ msg[8] +'\');">[回复]</a>'
			if (msg[3] == 0) {
				replycmt = '';
			}
			...
			if (ajaxtype == 'index') {
				GE('newcomment').innerHTML = '<dl id="' + msg[1] + '">' + replycmt + delcmt +' <dt><a href="blog.php?uid=' + msg[3] + '"><img src="' + msg[2] + '" class="authorPic" /></a><a href="blog.php?uid=' + msg[3] + '" class="b blue">' + msg[4] + '</a> ' + msg[5] + ' 回复： </dt> <dd>' + msg[6] + '</dd> </dl>' + GE('newcomment').innerHTML;
...
function ShowCmtReplyBox(uid,id,itemid,type){
	if (GE('r_' + id).style.display == '') {
		GE('r_' + id).style.display = 'none';
		GE('s_' + id).style.display = '';
		GE('s_' + id).innerHTML = '<div class="re-comment"> <b>回复：</b> <div><textarea class="ip" id="replytext" rows="10" style="width:93%">' + GE('r_content_' + id).innerHTML + '</textarea></div> <input valign="middle" class="bt" type="button" value="回复" onclick="ReplyCmt(\'' + uid + '\',\'' + id + '\',\'' + itemid + '\',\'' + type + '\')"> </div>';
	} else {
		GE('r_' + id).style.display = '';
		GE('s_' + id).style.display = 'none';
		GE('s_' + id).innerHTML = '';
...
function ReplyCmt(uid,id,itemid,type){
	if (GE('replytext').value == '') {
		alert('回复内容不能为空！');
		return false;
	}
	var url	  = ajaxurl + '?action=replycmt';
	var param = 'replytext=' + ajax_convert(GE('replytext').value) + '&uid=' + ajax_convert(uid) + '&id=' + ajax_convert(id) + '&itemid=' + ajax_convert(itemid) + '&type=' + ajax_convert(type);
	send_request(url,ReplyCmtResponse,param);
}

function ReplyCmtResponse(){
	var msg = http_request.responseText;
	msg = msg.split("\t");
	if (msg[0] == 'g_empty') {
		alert('回复内容不能为空！');
...
function TransferArticleBox(obj,id){
	open_menu(obj,id,2);
}

function TransferArticle(itemid){
	if (!winduid) {
		alert('您还没有登陆或注册，暂时不能使用此功能！');
		return false;
	}
	var cid = GE('transfer_cid').value;
	var dirid = GE('transfer_dirid').value;
	var url	  = ajaxurl + '?action=transferarticle';
	var param = 'cid=' + cid + '&dirid=' + dirid + '&itemid=' + itemid;
	send_request(url,TransferArticleResponse,param);
}

function TransferArticleResponse(){
	var msg = http_request.responseText;
	if(msg == 'success'){
		alert('转载成功！');
		closep();
